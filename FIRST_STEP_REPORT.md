...
- **Standing rule going forward (verbatim from the review):** _"post-processing the generated file is a liability we paid down once already. New requirements belong in jaeger-idl, or as refinements layered in schemas.ts — not as regexes over generated text."_ This governs all future work on this track, including the `lfx-week1` refinement layer (which complies: it never touches the generated file).
...
### 10.1 Next good step after 4438 merges

1. **Rebase this branch:** `git checkout lfx-week1-validated-wire-contract && git fetch upstream && git rebase upstream/main` — picks up the landed 13 re-exports; the `refined*` + `GetTraceResponseSchema` + `fetchTrace` layer stays on top (it never touched the generated file, so no strip conflicts by construction).
